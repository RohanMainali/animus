...
const CardiacScanScreen = ({ navigation, addHistoryEntry, isDarkMode }) => {
  ...
  // Function to update general scan streak and count
  const updateScanMetrics = async () => {
    try {
      ...
      if (lastScanDate === today) {
        ...
        if (lastScanDate === yesterdayString) {
          currentScanStreak += 1 // Continue streak
        } else {
          currentScanStreak = 1 // Reset streak
        }
      } else {
        currentScanStreak = 1 // First scan
      }
      ...
      console.log(`Scan metrics updated: Streak ${currentScanStreak}, Total ${totalScans}`)
    } catch (e) {
      console.error("Failed to update scan metrics:", e)
    }
  }
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const newEntry = {
        ...
        // Ensure audio_features exists if waveformUrl is expected by ResultsScreen
        audio_features: {
          waveformUrl: result.scanData.waveformUrl,
          spectrogramUrl: result.scanData.spectrogramUrl,
        },
        ...result,
      }
      addHistoryEntry(newEntry)
      await updateScanMetrics() // Update general scan streak and count
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      <Card>
        <Text style={[styles.cardTitle, { color: colors.text }]}>Heartbeat Recording</Text>
